...
async def invoke_doc_agent(user_input: str):
                    ...
                    if tool_name == 'search_documentation':
                        ...
                        if content_length > 1000:
                            with st.expander("📄 읽은 문서 내용 미리보기", expanded=False):
                                ...
                                st.text_area("문서 내용", value=result_str[:500] + "...", height=200, disabled=True, label_visibility="collapsed")
    ...
    # 내용이 비어있으면 최종 응답 생성 시도
    if not current_response.content and iteration >= max_iterations:
        status_text.text("🤖 최종 답변을 생성하고 있습니다...")
        try:
            final_messages = messages + [SystemMessage(content="지금까지 수집한 정보를 바탕으로 한국어로 답변을 작성해주세요. 반드시 참고 문서 링크를 포함해주세요.")]
            current_response = await model.ainvoke(final_messages)
        except Exception as e:
    ...
    if current_response.content:
        ...
        return "죄송합니다. 문서를 읽었지만 답변 생성에 실패했습니다. 다시 시도해주세요."
